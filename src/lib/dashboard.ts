...
export async function getDashboard(userId: string, range: DateRange) {
  const [cuentas, cats, groups] = await Promise.all([
  ...
  const ingresosMes = await getIngresoQuincena(userId, anio, mes, quincena);
  const regla = await getReglaPct(userId);
  ...
  const [apartadosActivos, contribsQuincena, recientesRaw] = await Promise.all([
    db.select().from(apartados).where(and(eq(apartados.activo, true), eq(apartados.userId, userId))).execute(),
    db
      .select()
  ...
  const recientes = [];
  for (const t of recientesRaw) {
    const cat = t.categoryId ? catById.get(t.categoryId) : undefined;
    let apartadoNombre: string | null = null;
    if (t.apartadoId) {
      const ap = await db
        ...
        .execute();
      apartadoNombre = ap[0]?.nombre ?? null;
    }
    recientes.push({
      id: t.id,
